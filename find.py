...
from regexes import (
    ...
    INFRA_ANTECEDENT_REGEX, 
    ANTE_ANTECEDENT_REGEX,
    MPEP_ANTECEDENT_REGEX
)
...
def get_citations(
    ...
    `models.CitationBase` objects representing the citations found
    in the document.

    Args:
        plain_text: The text to parse. You may wish to use the
            `clean.clean_text` function to pre-process your text
        ...
        tokenizer: An instance of a Tokenizer object. See `tokenizers`
            for information about available tokenizers. Uses the
            `tokenizers.AhocorasickTokenizer` by default.

    Returns:
        A list of `models.CitationBase` objects
    """
    ...
    for i, token in citation_tokens:
        ...
        # CASE 3a: Token is a "infra" reference.
        # In this case, we're not sure yet what the citation's antecedent is.
        # It could be any of the previous citations above. Thus, like an Id.
        # citation, for safety we won't resolve this reference yet.
        elif token_type is InfraToken:
            citation = _extract_infra_citation(words, i)
        
        # CASE 3b: Token is a "ante" reference.
        # In this case, we're not sure yet what the citation's antecedent is.
        # It could be any of the previous citations above. Thus, like an Id.
        # citation, for safety we won't resolve this reference yet.
        elif token_type is AnteToken:
            citation = _extract_ante_citation(words, i)

        # CASE 3c: Token is a "MPEP" reference.
        # In this case, we're not sure yet what the citation's antecedent is.
        # It could be any of the previous citations above. Thus, like an Id.
        # citation, for safety we won't resolve this reference yet.
        elif token_type is MPEPToken:
            citation = _extract_shortform_citation(words, i)
...
def _extract_infra_citation(
    words: Tokens,
    index: int,
) -> InfraCitation:
    """Given a list of words and the index of a infra token, look before
    and after to see if this is a infra citation. If found, construct
    and return a InfraCitation object.

    Infra 1: Adarand, infra, at 240
    Infra 2: Adarand, 515 infra, at 240
    Infra 3: Adarand, infra, somethingelse
    Infra 4: Adrand, infra. somethingelse
    """
    pin_cite, span_end, parenthetical = extract_pin_cite(words, index)
    antecedent_guess = None
    volume = None
    ...
    # Return InfraCitation
    return InfraCitation(
        cast(InfraToken, words[index]),
        index,
        span_end=span_end,
        metadata={
            "antecedent_guess": antecedent_guess,
...
def _extract_ante_citation(
    words: Tokens,
    index: int,
) -> AnteCitation:
    """Given a list of words and the index of a ante token, look before
    and after to see if this is a ante citation. If found, construct
    and return a AnteCitation object.

    Ante 1: Adarand, ante, at 240
    Ante 2: Adarand, 515 ante, at 240
    Ante 3: Adarand, ante, somethingelse
    Ante 4: Adrand, ante. somethingelse
    """
    pin_cite, span_end, parenthetical = extract_pin_cite(words, index)
    antecedent_guess = None
    volume = None
...
#def _extract_mpep_citation(
  #  words: Tokens,
  # index: int,
#) -> MPEPCitation:
    """Given a list of words and the index of an MPEP token, look before
    and after to see if this is a MPEP citation. If found, construct
    and return a MPEPCitation object.
