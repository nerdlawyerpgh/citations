...
class CitationBase:
    """Base class for objects returned by "find.get_citations`. We
...
class FullLawCitation(FullCitation):
    ...
    def add_metadata(self, words: "Tokens"):
        ...
        from helpers import add_law_metadata
...
class FullJournalCitation(FullCitation):
    ...
    def add_metadata(self, words: "Tokens"):
        """Extract metadata from text before and after citation."""
        # pylint: disable=import-outside-toplevel
        from helpers import add_journal_metadata
...
class FullCaseCitation(CaseCitation, FullCitation):
    ...
    def add_metadata(self, words: "Tokens"):
        """Extract metadata from text before and after citation."""
        # pylint: disable=import-outside-toplevel
        from helpers import add_defendant, add_post_citation
...
@dataclass(eq=False, unsafe_hash=False, repr=False)
class InfraCitation(CitationBase):
    """Convenience class which represents a 'infra' citation, i.e., a citation
    to something that is below in the document. Like a short form citation,
    this kind of citation lacks a full case name and usually has a different
    page number than the canonical citation.
    ...
    Examples:
    ```
    Adarand, infra, at 240
    Adarand, 515 infra, at 240
    Adarand, infra, somethingelse
    Adarand, infra. somethingelse
    ```
    """
...
@dataclass(eq=False, unsafe_hash=False, repr=False)
class AnteCitation(CitationBase):
    """Convenience class which represents a 'ante' citation, i.e., a citation
    to something that is before in the document. Like a short form citation,
    this kind of citation lacks a full case name and usually has a different
    page number than the canonical citation.
    ...
    ```
    Adarand, ante, at 240
    Adarand, 515 ante, at 240
    Adarand, ante, somethingelse
    Adarand, ante. somethingelse
    ```
    """
...
@dataclass(eq=False, unsafe_hash=False, repr=False)
class MPEPCitation(CitationBase):
    """Convenience class which represents an 'MPEP' citation, i.e., a citation
    to the MPEP. Like a short form citation, this kind of citation includes a chapter
    number or other pinpoint citation to the mpep schema.
...
@dataclass(eq=True, unsafe_hash=True)
class InfraToken(Token):
    """Word matching "infra" with or without punctuation."""

@dataclass(eq=True, unsafe_hash=True)
class AnteToken(Token):
    """Word matching "ante" with or without punctuation."""

@dataclass(eq=True, unsafe_hash=True)
class MPEPToken(Token):
    """Word matching "mpep" with or without punctuation."""
...
class Resource(ResourceType):
    ...
    resolved. See `resolve` for more details."""
